...
final class UplineAuthController extends Controller
{
    ...
    {
        try {
            $refreshTokenString = $request->refresh_token ?? $request->cookie('upline_refresh_token');

            if (! $refreshTokenString) {
                return $this->error('Refresh token not provided', 401);
            }

            $command = new RefreshTokenCommand($refreshTokenString, 'upline');
    ...
    public function logout(Request $request): JsonResponse
    {
        try {
            ...
            if ($jwtToken instanceof JWTToken) {
                $command = new LogoutUserCommand($jwtToken, 'upline');
            ...
            // Clear cookies
            return $response->withCookie(
                Cookie::forget('upline_token')
            )->withCookie(
                Cookie::forget('upline_refresh_token')
            );
    ...
    public function profile(Request $request): JsonResponse
    {
        try {
            ...
            $result = $this->getUserProfileUseCase->execute($jwtToken, 'upline');
